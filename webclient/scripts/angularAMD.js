...
define(function () {
    ...
    function checkBootstrapped() {
        if ( !bootstrapped ) {
            throw Error("angularAMD not initialized.  Need to call angularAMD.bootstrap(webclient) first.");
        }
    }
     ...
     * object into the current ng-webclient.  As result, if there are subsequent call to retrieve the
     * module post processQueue, it would retrieve a module that is not integrated into the ng-webclient.
     * 
    ...
    /**
     * Expose name of the webclient that has been bootstrapped
     */
    ...
    /**
     * Recreate the modules created by alternate angular in ng-webclient using cached $provider.
    ...
    /**
     * Return cached webclient provider
     */
     ...
     * Designed to simulate webclient.config.
     */
     ...
     * $provider and $injector from the webclient to be used later.
     *
     * enable_ngload: 
     */
    angularAMD.prototype.bootstrap = function (app, enable_ngload, elem) {
        ...
        // Store reference to original angular and webclient
        orig_angular = angular;

        // Create new version of webclient
        orig_app = app;
        alt_app = {};
        ...
        app.config(
            ['$controllerProvider', '$compileProvider', '$filterProvider', '$animateProvider', '$provide', '$injector', function (controllerProvider, compileProvider, filterProvider, animateProvider, provide, injector) {
                ...
                // Substitue provider methods from webclient call the cached provider
        ...
        // Get the injector for the webclient
        ...
        // Store the webclient name needed by .bootstrap function.
        app_name = app.name;
        ...
        // Create a webclient.register object to keep backward compatibility
